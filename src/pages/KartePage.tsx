...
// 五十音ユーティリティは lib/kana.ts に集約（予約作成のワンちゃん選択と共用）。
// 既存 import 互換のため hasKanji はここから再エクスポートする。
import { hasKanji, kanaGroups, kanaKey } from '../lib/kana';
export { hasKanji };
...
function KarteInner({ tenantId }: { tenantId: string }) {
  ...
  const groups = useMemo(
    () => kanaGroups(visibleDogs, (d) => kanaKey(d.name, d.nameKana)).map((g) => ({ row: g.row, dogs: g.items })),
    [visibleDogs],
  );
